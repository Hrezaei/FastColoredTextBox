...
namespace Tester
{
    public partial class ParsisSample : Form
    {
        string lang = "Parsis (custom highlighter)";
        ...
        private void ParsisSyntaxHighlight(TextChangedEventArgs e)
        {
            fctb.LeftBracket = '(';
            fctb.RightBracket = ')';
            fctb.LeftBracket2 = ':';
            ...
            //string highlighting
            e.ChangedRange.SetStyle(GreenStyle, new Regex(@"""""|@""""|''|«.*?»|(?<!@)(?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexOptions.Singleline));
            ...
            e.ChangedRange.SetStyle(BlueStyle, new Regex(@"\b(به نام خدا|برنامه|شروع|منطقی|پایان|متغیر|بازگشت|تابع|و|ازنوع|از|تا|بود|تاوقتی|هست|رشته|do|double|وگرنه|enum|event|explicit|extern|نادرست|finally|fixed|float|برای|foreach|goto|اگر|implicit|in|صحیح|interface|internal|is|lock|long|namespace|new|null|object|operator|out|override|params|private|protected|public|readonly|ref|برگردان|sbyte|sealed|short|sizeof|stackalloc|static|string|struct|switch|this|throw|درست|try|typeof|uint|ulong|unchecked|unsafe|ushort|using|virtual|void|volatile|while|add|alias|ascending|descending|dynamic|from|get|global|group|into|join|let|orderby|partial|remove|select|set|value|var|where|yield)\b|#region\b|#endregion\b"));
            ...
            e.ChangedRange.SetFoldingMarkers(@"شروع\b", @"پایان\b");//allow to collapse begin blocks
        ...
        private void miParsis_Click(object sender, EventArgs e)
        {
            //set language
            ...
            switch (lang)
            {
                ...
                case "Parsis (custom highlighter)":
                    ...
                    fctb.OnTextChanged();
                    break;
                case "Parsis": fctb.Language = Language.Parsis; break;
            ...
            miChangeColors.Enabled = lang != "Parsis (custom highlighter)";
        }
